...
CASES = [('PORTAL-001',
  'durable issuance requires human session',
  'datum_sync/portal_core.py',
  '    if operator and not user.is_operator:',
  '    if False:',
  'tests/test_portal.py::test_agent_cannot_mint_or_approve'),
 ('PORTAL-002',
  'baseline tier cannot write',
  'datum_sync/portal_core.py',
  '    if user.tier < tier:',
  '    if False:',
  'tests/test_portal.py::test_baseline_denied_write_then_elevation_allows'),
 ('PORTAL-003',
  'refresh preserves absolute deadline',
  'datum_sync/portal_core.py',
  "    deadline = authorization['authorized_until']",
  '    deadline = now() + timedelta(days=30)',
  'tests/test_portal.py::test_refresh_deadline_and_family_reuse'),
 ('PORTAL-004',
  'restriction lowers tier without deleting grant',
  'datum_sync/portal_core.py',
  "    if row['portal_state'] == 'restricted': tier = min(tier,1)",
  '    if False: tier = min(tier,1)',
  'tests/test_portal.py::test_restrict_preserves_deny_and_restore_keeps_current_grant'),
 ('PORTAL-005',
  'pending execution rechecks current grant',
  'datum_sync/portal.py',
  "if user.tier<3 or snapshot['tier']<3 or not user.permits('publish',args['path']):",
  "if user.tier<3 or snapshot['tier']<3:",
  'tests/test_portal.py::test_pending_call_rechecks_authority_and_executes_once'),
 ('PORTAL-006',
  'concurrent session admission bounded',
  'datum_sync/portal.py',
  '            if count>=limit:',
  '            if False:',
  'tests/test_portal.py::test_sessions_are_atomic_and_bound_to_credential'),
 ('PORTAL-007',
  'operator mutations require CSRF',
  'datum_sync/portal_core.py',
  "        if not secrets.compare_digest(request.headers.get('x-csrf-token',''), expected):",
  '        if False:',
  'tests/test_portal.py::test_cookie_csrf_and_cross_origin_refused'),
 ('FED-001',
  'federated catalogue enforces exact principal grant',
  'datum_sync/federation.py',
  ...
  "                and await credential_access.access_grant(conn,user,row['connection'],row['tool_name'])):",
  'tests/test_mcp_federation.py::test_missing_mcp_grant_hides_tool_and_never_forwards'),
 ('FED-002',
  'MCP grant tool names cannot contain wildcards',
  'datum_sync/portal_core.py',
  ...
  "if not isinstance(patterns, list) or len(patterns) > 50 or any(not isinstance(v,str) for v in patterns):",
  'tests/test_mcp_federation.py::test_mcp_grant_rejects_wildcard_tool_names'),
 ('FED-011',
  'configured caller credentials cannot cross upstream boundary',
  'datum_sync/federation.py',
  "    headers = {str(k):str(v) for k,v in (config.get('headers') or {}).items()\n               if str(k).lower() not in ('authorization','cookie','host','connection')}",
  "    headers = {str(k):str(v) for k,v in (config.get('headers') or {}).items()}",
  'tests/test_mcp_federation_credentials.py::test_configured_caller_authorization_is_replaced_by_upstream_secret'),
 ('FED-013',
  'federated call audit excludes tool arguments',
  'datum_sync/federation.py',
  "                 detail={'tool':row['tool_name']})",
  "                 detail={'tool':row['tool_name'],'arguments':arguments})",
  'tests/test_mcp_federation.py::test_granted_federated_tool_is_listed_and_forwarded'),
 ('FED-020',
  'gateway rejects oversized MCP request before forwarding',
  'datum_sync/portal.py',
  '            if size>262144:',
  '            if False:',
  'tests/test_mcp_federation.py::test_arguments_are_capped_before_forwarding'),
 ('MCP-020',
  'projected upstream tool names are bounded',
  'datum_sync/federation.py',
  '    if len(clean) <= 48:',
  '    if True:',
  'tests/test_mcp_federation.py::test_catalogue_refresh_is_persistent_and_tool_names_are_bounded'),
 ('JOB-001',
  'gateway supplies trusted principal context to upstream job tools',
  'datum_sync/federation.py',
  "            '_meta':{'io.datum.principal':prepared['principal']}})",
  "            '_meta':prepared['arguments'].get('_meta',{})})",
  'tests/test_mcp_federation.py::test_job_status_and_logs_are_scoped_to_server_generated_principal'),
 ('JOB-002',
  'job reads are limited to caller and descendant principals',
  'datum_sync/legacy_mcp.py',
  '            ownership = "(portal_principal_id=ANY($1::int[]) OR (portal_principal_id IS NULL AND submitted_by=ANY($2::text[])))"',
  '            ownership = "($1::int[] IS NOT NULL OR $2::text[] IS NOT NULL)"',
  'tests/test_mcp_federation.py::test_job_status_and_logs_are_scoped_to_server_generated_principal'),
 ('JOB-003',
  'job reads are limited to granted repositories',
  'datum_sync/legacy_mcp.py',
  '                      AND ($3::boolean OR repository=ANY($4::text[])) AND ($5::text IS NULL OR status=$5)',
  '                      AND ($3::boolean OR $4::text[] IS NOT NULL) AND ($5::text IS NULL OR status=$5)',
  'tests/test_mcp_federation.py::test_job_status_and_logs_are_scoped_to_server_generated_principal'),
 ('JOB-004',
  'job log pages obey the requested maximum',
  'datum_sync/legacy_mcp.py',
  ...
  '            entries = await conn.fetch("""SELECT id,ts,level,left(message,4096) AS message FROM job_log\n                WHERE job_id=$1 AND id>$2 ORDER BY id LIMIT ($3 + 1000)""", job_id, after_id, limit)',
  'tests/test_mcp_federation.py::test_job_status_and_logs_are_scoped_to_server_generated_principal'),
 ('JOB-005',
  'individual log messages are truncated before federation',
  'datum_sync/legacy_mcp.py',
  '            entries = await conn.fetch("""SELECT id,ts,level,left(message,4096) AS message FROM job_log',
  '            entries = await conn.fetch("""SELECT id,ts,level,message FROM job_log',
  'tests/test_mcp_federation.py::test_job_status_and_logs_are_scoped_to_server_generated_principal'),
 ('JOB-006',
  'authenticated submissions persist the stable principal id',
  'datum_sync/jobs.py',
  '            principal_id,\n        )',
  '            None,\n        )',
  'tests/test_api.py::test_the_job_listing_returns_a_submitted_job'),
 ('JOB-007',
  'job notifications are persisted as durable events',
  'datum_sync/jobs.py',
  '    event_id = await conn.fetchval(\n        "INSERT INTO job_events(job_id,kind,payload) VALUES($1,$2,$3) RETURNING id",\n        job_id, kind, json.dumps(payload),\n    )',
  '    event_id = 0',
  'tests/test_jobs.py::test_oversized_notify_payload_does_not_raise'),
 ('JOB-008',
  'gateway principal context carries effective repository grants',
  'datum_sync/federation.py',
  "            'principal':{'id':user.id,'name':user.name,'repositories':user.repositories()}}",
  "            'principal':{'id':user.id,'name':user.name,'repositories':['*']}}",
  'tests/test_mcp_federation.py::test_job_status_and_logs_are_scoped_to_server_generated_principal'),
 ('JOB-009',
  'artifact reads strip internal file and path fields',
  'datum_sync/legacy_mcp.py',
  '                safe = [{key:item[key] for key in ("name","type","primary","dir","size") if key in item}\n                        for item in raw if isinstance(item,dict)]',
  '                safe = raw',
  'tests/test_mcp_federation.py::test_job_status_and_logs_are_scoped_to_server_generated_principal'),
 ('JOB-010',
  'durable event pages obey the requested maximum',
  'datum_sync/legacy_mcp.py',
  '                entries = await conn.fetch("""SELECT id,kind,payload,created_at FROM job_events\n                    WHERE job_id=$1 AND id>$2 ORDER BY id LIMIT $3""",job_id,after_id,limit)',
  '                entries = await conn.fetch("""SELECT id,kind,payload,created_at FROM job_events\n                    WHERE job_id=$1 AND id>$2 ORDER BY id LIMIT ($3 + 1000)""",job_id,after_id,limit)',
  'tests/test_mcp_federation.py::test_job_status_and_logs_are_scoped_to_server_generated_principal'),
 ('MCPFLOW-001',
  'every MCP request starts with a durable received event',
  'datum_sync/mcp_observe.py',
  '            await _event(conn, trace_id, "request.received",\n                         {"http_method": _short(http_method, 16), "request_bytes": max(0, request_bytes)})',
  '',
  'tests/test_mcp_observability.py::test_federated_call_has_one_deterministic_payload_free_timeline'),
 ('MCPFLOW-002',
  'MCP flow events never store tool arguments',
  'datum_sync/portal.py',
  "                               detail={'method':str(method)[:80],\n                                       'tool':str(name)[:200] if name is not None else None})",
  "                               detail={'arguments':params.get('arguments')})",
  'tests/test_mcp_observability.py::test_federated_call_has_one_deterministic_payload_free_timeline'),
 ('MCPFLOW-003',
  'invalid bearer attempts finish as denied flows',
  'datum_sync/portal.py',
  "        return await complete({'detail':exc.detail},exc.status_code,\n                              outcome='denied',error_code=code)",
  "        return await complete({'detail':exc.detail},exc.status_code,\n                              outcome='ok',error_code=code)",
  'tests/test_mcp_observability.py::test_invalid_bearer_is_anonymous_but_still_recorded'),
 ('MCPFLOW-004',
  'the response trace id is the durable MCP flow id',
  'datum_sync/portal.py',
  '    trace=request.state.trace',
  '    trace=uuid.uuid4()',
  'tests/test_mcp_observability.py::test_federated_call_has_one_deterministic_payload_free_timeline'),
 ('MCPFLOW-005',
  'federated completion remains on the inbound request trace',
  'datum_sync/portal.py',
  "            await mcp_observe.mark(trace,'upstream.completed',\n                                   detail={'is_error':is_error,'outcome':outcome})",
  "            await mcp_observe.mark(uuid.uuid4(),'upstream.completed',\n                                   detail={'is_error':is_error,'outcome':outcome})",
  'tests/test_mcp_observability.py::test_federated_call_has_one_deterministic_payload_free_timeline'),
 ('MCPFLOW-006',
  'valid principals remain attributable when session validation fails',
  'datum_sync/portal.py',
  "        if user is not None:\n            await mcp_observe.mark(trace,'auth.accepted',principal=user,\n                                   detail={'credential_kind':user.credential['kind']})",
  '        if False:\n            pass',
  'tests/test_mcp_observability.py::test_valid_principal_with_missing_session_is_attributable'),
('SERVER-001',
 'disabled MCP servers disappear from agent discovery immediately',
 'datum_sync/federation.py',
 ...
 "        JOIN connections c ON c.name=t.connection WHERE c.type='mcp'\n"
 "          AND t.enabled=true ORDER BY t.tool_name''')",
 'tests/test_mcp_server_registry.py::test_tool_and_server_revocation_change_next_agent_catalogue'),
('SERVER-002',
 'disabled MCP tools disappear from agent discovery immediately',
 'datum_sync/federation.py',
 "          AND t.enabled=true ORDER BY t.tool_name''')",
 "          ORDER BY t.tool_name''')",
 'tests/test_mcp_server_registry.py::test_tool_and_server_revocation_change_next_agent_catalogue'),
('SERVER-003',
 'per-agent tool revocation removes only the selected tool',
 'datum_sync/portal.py',
 "        remaining=[tool for tool in grant['allowed_tools'] if tool!=tool_name]",
 "        remaining=list(grant['allowed_tools'])",
 'tests/test_mcp_server_registry.py::test_tool_and_server_revocation_change_next_agent_catalogue'),
('CRED-001',
 'operator dashboard never selects credential ciphertext',
 'datum_sync/portal.py',
 'c.created_at,c.updated_at,v.version,v.key_id,v.fingerprint,v.secret_fields,v.activated_at',
 'c.created_at,c.updated_at,v.version,v.key_id,v.fingerprint,v.secret_fields,v.activated_at,v.ciphertext',
 'tests/test_credential_access.py::test_request_approve_use_and_revoke_is_immediate')
, ('CRED-002',
 'every federated call requires a live credential grant',
 'datum_sync/federation.py',
 '    if not credential:\n'
 "        fail(403,'CREDENTIAL_ACCESS_REQUIRED','Request access to this upstream credential before calling "
 "the tool.')",
 '    if False:\n        pass',
 'tests/test_credential_access.py::test_request_approve_use_and_revoke_is_immediate')
, ('CRED-003',
 'credential access revocation is immediately persisted',
 'datum_sync/portal.py',
 "UPDATE credential_grants SET state='revoked',revoked_by=$2,revoked_at=now(),revoke_reason=$3 WHERE id=$1",
 "UPDATE credential_grants SET state='active',revoked_by=$2,revoked_at=now(),revoke_reason=$3 WHERE id=$1",
 'tests/test_credential_access.py::test_request_approve_use_and_revoke_is_immediate')
, ('CRED-004',
 'an operator cannot expand a credential request while approving it',
 'datum_sync/portal.py',
 "        if not set(tools)<=set(pending['requested_tools']): fail(400,'GRANT_EXCEEDS_REQUEST')",
 '        if False: pass',
 'tests/test_credential_access.py::test_approval_cannot_expand_request_and_disable_stops_all_access')
, ('CRED-005',
 'a failed credential probe never activates the staged version',
 'datum_sync/portal.py',
 "            await credential_access.fail_stage(conn,current,staged,'UPSTREAM_TEST_FAILED')",
 '            await credential_access.activate(conn,current,staged)',
 'tests/test_credential_access.py::test_rotation_tests_before_activation_and_never_returns_secret')
, ('CRED-006',
 'credential disablement makes every grant ineffective',
 'datum_sync/credential_access.py',
 ...
 "    credential_live=bool(credential and credential['server_state']=='active'\n"
 "                         and principal and credential['server_owner_id']==principal['portal_parent']\n"
 "                         and (credential['expires_at'] is None or credential['expires_at']>now()))",
 'tests/test_credential_access.py::test_approval_cannot_expand_request_and_disable_stops_all_access')
, ('CRED-007',
 'credential probe failures never echo upstream text',
 'datum_sync/portal.py',
 "        fail(502,'CREDENTIAL_TEST_FAILED','The replacement credential was rejected; the current version "
 "remains active.')",
 "        fail(502,'CREDENTIAL_TEST_FAILED',str(exc))",
 'tests/test_credential_access.py::test_rotation_tests_before_activation_and_never_returns_secret')
] + [
    (
        "AUTH-001",
    ...
    (
        ...
        "the login lockout",
        "datum_sync/auth.py",
        # Both the preflight and post-semaphore checks use this decision.
